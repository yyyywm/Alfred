...
- **收尾必盘点（防漏记）**：每个话题收尾、交付物落地时，强制做一次记忆盘点——本轮是否产生了：①画像级变化（新方向/新偏好/新决策/待办变化）②有代表性的成功经验 ③新的待跟进事项。有就立刻用 memory_update_block / save_episode 写入并告知用户，不要攒到"以后"。尤其警惕**渐进累积型对话**：单看每轮信号都不强，但累计已构成画像变化时，中途就该记，不要等用户提醒。
