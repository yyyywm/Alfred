...
- 首轮对话完成后，Alfred 会在后台自动概括一个会话标题（≤15 字），`/sessions` 列表和恢复会话时可见；`/title` 可手动改名，手动标题不会被自动概括覆盖
...
| `/memory` | 查看长期记忆 |
| `/why` | 查看上一轮回答依据了哪些记忆 |
| `/sessions` | 列出历史会话（带序号与标题；无标题回退显示首条消息预览） |
| `/load <序号/id/标题关键词>` | 加载历史会话，继续之前的上下文（多匹配会列出候选供选择） |
| `/delete <序号/id/标题关键词>` | 删除会话记录（需确认） |
| `/title <标题>` | 手动设置当前会话标题（截断 30 字符；无参数查看当前标题） |
